...
import { BookOpen, ClipboardList, Calendar, BarChart3, Bell, User, Settings, LogOut, CreditCard } from 'lucide-react';
...
export default function StaffSidebar() {
  ...
  const menuItems = [
    { id: 'kelola-buku', label: 'Kelola Buku', icon: BookOpen, color: 'blue', href: '/petugas/dashboard/kelola-buku' },
    { id: 'permintaan', label: 'Permintaan', icon: ClipboardList, color: 'indigo', href: '/petugas/dashboard/permintaan' },
    { id: 'peminjaman-aktif', label: 'Peminjaman Aktif', icon: Calendar, color: 'cyan', href: '/petugas/dashboard/peminjaman-aktif' },
    { id: 'verifikasi-denda', label: 'Verifikasi Denda', icon: CreditCard, color: 'orange', href: '/petugas/dashboard/verifikasi-denda' },
    { id: 'grafik', label: 'Grafik', icon: BarChart3, color: 'teal', href: '/petugas/dashboard/grafik' },
    { id: 'profil', label: 'Profil', icon: User, color: 'purple', href: '/petugas/dashboard/profil' },
    { id: 'setting', label: 'Setting', icon: Settings, color: 'gray', href: '/petugas/dashboard/setting' },
  ];

  const getActivePage = () => {
    if (pathname?.includes('kelola-buku')) return 'kelola-buku';
    if (pathname?.includes('permintaan')) return 'permintaan';
    if (pathname?.includes('peminjaman-aktif')) return 'peminjaman-aktif';
    if (pathname?.includes('verifikasi-denda')) return 'verifikasi-denda';
    if (pathname?.includes('grafik')) return 'grafik';
      ...
      <div className="p-6">
        ...
        <div className="mb-8 pb-6 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Petugas Panel</h2>
